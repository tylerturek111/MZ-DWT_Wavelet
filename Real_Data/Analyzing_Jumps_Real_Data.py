...
# Anomaly Threshold (anomaly threshold = noise * ratio)
ratio = 50
...
detector_start = 0
detector_seperation = 1
detector_count_sqrt = 4
...
fig1, axs1 = plt.subplots(detector_count_sqrt, detector_count_sqrt, figsize=(15, 15))
fig2, axs2 = plt.subplots(detector_count_sqrt, detector_count_sqrt, figsize=(15, 15))
fig3, axs3 = plt.subplots(detector_count_sqrt, detector_count_sqrt, figsize=(15, 15))
fig4, axs4 = plt.subplots(detector_count_sqrt, detector_count_sqrt, figsize=(15, 15))

# Setting up the time axis
time_axis = np.linspace(0, time_end - time_start, time_end - time_start, endpoint=False)
time_axis2 = np.linspace(0, time_end - time_start, time_end - time_start - smoothing_index + 1, endpoint=False)
summary_stats = []
...
if os.path.exists(jump_plots_path):
    shutil.rmtree(jump_plots_path)
...
if os.path.exists(summary_plots_path):
    shutil.rmtree(summary_plots_path)
...
for i in range(detector_count_sqrt):
    for j in range(detector_count_sqrt):
        # The current detector
        detector = detector_start + (i * detector_count_sqrt + j) * detector_seperation

        # Setting up the output print
        title_text = f"For Detector Number {detector}"
        print("-------------------------------------------------------------")
        Singularity_Analysis.print_colored(title_text, "blue")

        # Getting the real data
        original_data, noise = Generating_Real_Data.get_real_data(time_start, time_end, detector)

        print(f"Noise Level for {i}, {j}: {noise}")
        ...
        deviation_data = original_data[0 : time_end - time_start - smoothing_index + 1] - smooth_data
        ...
        deviation_wavelet = first_wavelet_transform[0 : time_end - time_start - smoothing_index + 1] - smooth_wavelet
        ...
        axs1[i, j].set_title(f"Original Signal of Detector {detector}", fontsize = 8)
        ...
        axs2[i, j].set_title(f"Wavlet Transform of Detector {detector}", fontsize = 8)
        ...
        axs3[i, j].set_title(f"Alpha from Old Method of Detector {detector}", fontsize = 8)
        ...
        # 
        alpha_values, alpha_jump_indexes = Singularity_Analysis.packaged_compute_alpha_values_and_indexes(wavelet_transform, 1, jump_threshold, alpha)

        # Plotting the new alpha values
        axs4[i, j].plot(time_axis, alpha_values)
        axs4[i, j].set_title(f"Alpha from New Method of Detector {detector}", fontsize = 8)
        axs4[i, j].set_xlabel("Time (s)", fontsize = 6)
        axs4[i, j].set_ylabel("Alpha", fontsize = 6)
        axs4[i, j].tick_params(axis = "both", labelsize = 4)
        axs4[i, j].axhline(y = alpha, color = "r", linestyle = "--")
        axs4[i, j].axhline(y = -1 * alpha, color = "r", linestyle = "--")
        ...
        if combined.shape[0] != 0:
            ...
            for j in range(combined.size):
                ...
                ax5.set_title(f"Detector {detector} Around Point {combined[j]}")
                ...
                # Saving the plot
                file_name = f"{detector}_{combined[j]}"
